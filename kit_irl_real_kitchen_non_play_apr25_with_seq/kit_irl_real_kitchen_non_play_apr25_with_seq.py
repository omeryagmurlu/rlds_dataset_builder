...
seq_root_path = "/home/yagmurlu/data/flower_mp_crops_sequences"
seq_langs_map = {
    "flower_mp_crops_opencloseall_sequence": [
        ["Open the microwave", "Open the microwave door"],
        ["Open the oven", "Open the oven door"],
        ["Open the ice box", "Open the fridge"],
        ["Close the ice box", "Close the fridge"],
        ["Close the oven", "Close the oven door"],
        ["Close the microwave", "Close the microwave door"],
    ],
    "flower_mp_crops_oven_sequence": [
        ["Open the oven", "Open the oven door"],
        ["Pull the oven tray", "Pull the oven tray out"],
        ["Put the banana on the tray", "Move the banana to the tray"],
        ["Push the oven tray", "Push the oven tray in"],
        ["Close the oven", "Close the oven door"],
    ],
    "flower_mp_crops_stovetop_sequence": [
        ["Move the banana to the stove", "Put the banana on the stove"],
        ["Move the pot to the sink", "Put the pot into the sink"],
        ["Push the toaster lever", "Push the toaster down"],
        ["Put the toast on the stove", "Move the toast to the stove"],
    ]
}
...
def get_trajectories():
    ...
    for path, langs in seq_langs_map.items():
        episode_prep_path = os.path.join(seq_root_path, path)
        # get all episode paths
        episode_paths = glob.glob(os.path.join(episode_prep_path, '*'))
        for episode_path in episode_paths:
            with open(os.path.join(episode_path, 'segments'), 'r') as f:
                segments = f.readlines()
            segments = [0] + [int(x.strip()) for x in segments]
            segments = zip(segments[:-1], segments[1:])
            # for each segment:
            for seg_langs, segment in zip(langs, segments):
                # add language instruction to the path
                trajs.append({
                    'path': episode_path,
                    'langs': seg_langs,
...
def _parse_example(sample):
    ...
    langs = sample['langs']
    segment = sample.get('segment', None)
    ...
    for file in glob.glob(follower_path):
        ...
        if segment is not None:
            # if segment is provided, slice the data
            data[name] = data[name][segment[0]:segment[1]]
    ...
    top_cam_vector = create_img_vector(top_cam_path, trajectory_length, segment=segment)
    side_cam_vector = create_img_vector(side_cam_path, trajectory_length, segment=segment)
...
def create_img_vector(img_folder_path, trajectory_length, segment=None):
    cam_list = []
